...
EOF
  ...
  #sqlite3 ${WORK_DIR}/data/sqlite.db "update servers set secret='${LOCAL_TOKEN}' where id=71"
  LOCAL_TOKEN="YOxCAy863mk8CBwpz1"
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/derwalld/Argo-Nezha-Service-Container/main/template/backup.sh | sed '1,/^########/d' >> $WORK_DIR/backup.sh
...
EOF
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/derwalld/Argo-Nezha-Service-Container/main/template/restore.sh | sed '1,/^########/d' >> $WORK_DIR/restore.sh
  fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/derwalld/Argo-Nezha-Service-Container/main/template/renew.sh | sed '1,/^########/d' >> $WORK_DIR/renew.sh
